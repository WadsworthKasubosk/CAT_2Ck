...
def get_geometry_feature():
    ...
    # 过滤掉太小的轮廓（噪声），只保留面积>10的
    contours = [c for c in contours if cv2.contourArea(c) > 10]
    
    if not contours:
        # 没有有效轮廓，设置默认值
        c_features['area'].append(0)
        c_features['perimeter'].append(0)
        c_features['focus_x'].append(0)
        c_features['focus_y'].append(0)
        c_features['ellipse'].append(0)
        return
    ...
    try:
        if len(largest) >= 5:
            ...
            c_features['ellipse'].append(round(ma - MA, 4))
        else:
            c_features['ellipse'].append(0)
    except:
        c_features['ellipse'].append(0)
    ...
    # 面积周长
    c_features['area'].append(cv2.contourArea(largest))
    c_features['perimeter'].append(round(cv2.arcLength(largest, True), 4))
